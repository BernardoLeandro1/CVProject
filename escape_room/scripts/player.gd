...
func _process(delta):
	if Input.is_action_just_pressed("ui_crouch") and !crouched:
		crouched = true
		pcap.shape.height -= CROUCHING_SPEED * delta
		head.position.y = pcap.shape.height
	elif Input.is_action_just_pressed("ui_crouch") and crouched:
		crouched = false
		pcap.shape.height += CROUCHING_SPEED * delta
		head.position.y = pcap.shape.height
	
	pcap.shape.height = clamp(pcap.shape.height, CROUCH_HEIGHT, HEIGHT)
	var collider = interaction.get_collider()
	if collider != null:
...
func _physics_process(delta):
	...
	if direction and !crouched:
		velocity.x = direction.x * SPEED
		velocity.z = direction.z * SPEED
	elif direction and crouched:
		velocity.x = direction.x * CROUCHED_SPEED
		velocity.z = direction.z * CROUCHED_SPEED
	else:
